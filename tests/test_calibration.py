...
def test_save_and_load_calibration_round_trip(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "projects" / "skeleton").mkdir(parents=True)
...
def test_save_calibration_merges_over_existing_entry(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "projects" / "skeleton").mkdir(parents=True)

    calibration_module.save_calibration("skeleton", {"head": {"min": 10, "max": 170, "rest": 90}})
    calibration_module.save_calibration("skeleton", {"head": {"min": 20, "max": 160, "rest": 85}})

    assert calibration_module.load_calibration("skeleton") == {
        "head": {"min": 20, "max": 160, "rest": 85}
    }
