...
# We use apt for system packages, and --break-system-packages for global pip if needed
sudo apt update
sudo apt install -y python3-smbus python3-pip python3-websockets python3-dotenv

# For specific libraries not available via apt (like adafruit-circuitpython-servokit)
...
# 3. Create the systemd service using /bin/bash and wait for network online
